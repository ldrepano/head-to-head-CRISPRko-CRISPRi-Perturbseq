...
The analyze-singlecelldata directory includes all R and WDL scripts employed to analyze the data at single-cell resolution (in contrast to the SCEPTRE results, which summarize perturbation effects per guide). The data is not included in this repository due to size constraints. Raw single-cell RNA-seq data as fastq files as well as the results of SCEPTRE have been deposited at [GEO](link pending). Aligned, quality-filtered Perturb-seq datasets with guide assignments are available as RDS files at [Zenodo](https://doi.org/10.5281/zenodo.20722064).
